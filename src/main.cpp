...
void setup()
{
 // wifi_set_sleep_type(LIGHT_SLEEP_T); // for testing
  ...
  bool res = wm.autoConnect("RAIN-SENSOR-AP");
...
void getJsonData()
{
  ...
  GenJson["Value"] = RSensor.get.value;
  GenJson["wet_val"] = RSensor.get.wet_val;
...
bool sendtoMQTT()
{
  ...
  if (!_settings._mqttJson)
  {
    ...
    mqttclient.publish((topicStrg + "/Value").c_str(), dtostrf(RSensor.get.value, 4, 1, msgBuffer));
    mqttclient.publish((topicStrg + "/wet_val").c_str(), dtostrf(RSensor.get.wet_val, 4, 1, msgBuffer));
